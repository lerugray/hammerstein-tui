...
struct Cli {
    ...
    /// `sandbox_writable_roots` in config and `HAMMERSTEIN_SANDBOX_WRITABLE_ROOTS`
    /// (legacy alias `DEEPSEEK_SANDBOX_WRITABLE_ROOTS`) in the env. Use
    /// sparingly — every entry is full read+write for any command the model runs.
...
fn merge_project_config(config: &mut Config, workspace: &Path) {
    ...
    // `~/.deepseek/config.toml`, `HAMMERSTEIN_SANDBOX_WRITABLE_ROOTS`
    // (or legacy `DEEPSEEK_SANDBOX_WRITABLE_ROOTS`), or `--writable-root`
    // — surfaces the user controls directly.
