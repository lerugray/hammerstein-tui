...
- **Cross-repo writes**: Agent mode's shell tool can only write inside the workspace, `/tmp`, and `$TMPDIR` by default. To permit writes in sibling repos (e.g. fleet-dispatcher use cases like gs-scheduler firing `git pull` across multiple project directories), set `sandbox_writable_roots` in `~/.deepseek/config.toml`, the `HAMMERSTEIN_SANDBOX_WRITABLE_ROOTS` env var (legacy alias: `DEEPSEEK_SANDBOX_WRITABLE_ROOTS`), or `--writable-root <PATH>` on the CLI. Treat each entry like sudo — full read+write inside that subtree. Denied at project scope. See `docs/CONFIGURATION.md` for the blast-radius notes.
